...
/*   Updated: 2024/05/25 22:26:57 by jubaldo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_map(t_cub3d *game, char *line)
{
	...
	if (!map_lines)
		error_exit(game, "Error: Failed to allocate memory for map");
	map_lines[map_height - 1] = ft_strdup(line);
	if (!map_lines[map_height - 1])
	{
		free(map_lines);
		error_exit(game, "Error: Failed to allocate memory for map line");
	}
